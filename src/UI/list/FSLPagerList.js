 ...
 * @author xukj
 * @date 2018/10/9
 * @description 展示界面组件 FSLPagerList
...
export default class FSLPagerList extends React.PureComponent {
    static propTypes = {
        // 默认数据
        ...
        // v2
        // 分页加载更多的样式
        FSLListLoadMoreComponent: PropTypes.any,
        // 分页加载到最后的样式
        FSLListNoMoreComponent: PropTypes.any,
        // 加载完成后空界面.该空界面会覆盖掉header和footer,如果需要显示footer和header,请设置为null
        FSLListEmptyComponent: PropTypes.any,
    };
    ...
    static defaultProps = {
        ...
        // v2
        FSLListLoadMoreComponent: LoadMoreComponent,
        FSLListNoMoreComponent: NoMoreComponent,
    };
    ...
    _getFooterComponent = ({ refreshing, loading }) => {
        const {
            pager,
            FSLListLoadMoreComponent,
            FSLListNoMoreComponent,
        } = this.props;

        if (!loading && pager.isLastPage()) {
            return FSLListNoMoreComponent;
        }
        ...
        return FSLListLoadMoreComponent;
    };
