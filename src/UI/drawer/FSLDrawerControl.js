...
import FSLDrawerView from './FSLDrawerView';

// event
const hideType = 'REACT_NATIVE_KPFRAMEWORK_HIDE_DRAWER';

// static
class FSLDrawerController {
    hide = key => {
        DeviceEventEmitter.emit(hideType, key);
    };
}

const control = new FSLDrawerController();

export default class FSLDrawerControl extends React.PureComponent {
